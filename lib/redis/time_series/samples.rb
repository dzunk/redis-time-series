...
class Redis
  class TimeSeries
    class Samples < DelegateClass(Array)
      ...
      def self.sum_values!(calculated_samples:)
      calculated_samples.each { |sample| sample.value = sample.value.sum }
    end

      def self.subtract_values!(calculated_samples:)
      calculated_samples.each { |sample| sample.value = sample.value.reduce(sample.value.first * 2) { |result, next_value| result - next_value } }
    end

      def self.multiply_values!(samples:, factor:)
      samples.each { |sample| sample.value = sample.value * factor }
    end
    end
  end
end
